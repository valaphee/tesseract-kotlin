...
class LocationManager : BaseFacet<WorldContext, LocationManagerMessage>(LocationManagerMessage::class, Location::class) {
    override suspend fun receive(message: LocationManagerMessage): Response {
        message.source?.whenTypeIs<ActorType> {
            when (message) {
                is Move -> it.position += message.move.toMutableDouble3().rotate(it.rotation.x.toDouble(), Double3.YAxis)
                is MoveRotate -> {
                    it.rotation = message.rotation
                    it.position += message.move.toMutableDouble3().rotate(it.rotation.x.toDouble(), Double3.YAxis)
                }
