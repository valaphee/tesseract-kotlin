...
import com.valaphee.tesseract.world.chunk.terrain.generator.flat.FlatGenerator
...
class ChunkManager : BaseFacet<WorldContext, ChunkManagerMessage>(ChunkManagerMessage::class) {
    private val generator: Generator = FlatGenerator("minecraft:bedrock,59*minecraft:stone,3*minecraft:dirt,minecraft:grass_block,minecraft:snow;minecraft:snowy_tundra")
    ...
    override suspend fun receive(message: ChunkManagerMessage): Response {
        ...
        when (message) {
            ...
            is ChunkRelease -> context.world.removeEntities(context, message.source, *message.chunkPositions.map { chunks.remove(it)?.also { context.provider.saveChunk(it) }?.id }.filterNotNull().toLongArray())
        }
